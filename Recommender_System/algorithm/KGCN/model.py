...
def KGCN_model(n_user: int, n_entity: int, n_relation: int, adj_entity: List[List[int]], adj_relation: List[List[int]],
    ...
    l2 = tf.keras.regularizers.l2(l2)#L2 正则化器用于在模型训练中对权重进行 L2 正则化，以防止过拟合
    ...
    flatten = tf.keras.layers.Flatten()#将输入的多维数据压缩为一维
    ...
    for it in range(iter_size):
        #在非最后一层使用 ReLU 激活函数，在最后一层使用 tanh 激活函数
